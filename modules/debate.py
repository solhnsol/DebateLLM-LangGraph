...
async def human_negative_node(state: DebateState):
    # Interactive mode: This node is bypassed by update_state with as_node="human_negative"
    # If called directly, it implies no input was provided via interruption mechanism
    return {
        "messages": [],
        "last_script": "",
        "next_speaker": "moderator"
...
app = workflow.compile(checkpointer=checkpointer, interrupt_before=["human_negative"])
...
async def run_debate(session_id: int, chat_scripts:dict, current_node:Optional[str]):
        ...
        if msg.content:
            ...
            if metadata["langgraph_node"] == "human_negative":\
                ...
                yield {
                    ...
                    "content": parsed_content.get('script', '')
                    }
    ...
    async def main():
        ...
        while True:
            try:
                ...
                async for event in app.astream(current_inputs, config):
                    for node, values in event.items():
                        if "last_script" in values:
                            print(f"\n[{node}]: {values['last_script']}")
                            
                # Check if the graph is paused (waiting for input)
                state = app.get_state(config)
                if state.next:
                    if "human_negative" in state.next:
                        user_input = input("\n[Human Negative] 입력 (q to quit)> ")
                        if user_input.lower() == 'q':
                            break
                            
                        # Manually update state as if the human node executed
                        app.update_state(
                            config,
                            {
                                "messages": [HumanMessage(content=user_input, name="negative")],
                                "last_script": user_input,
                                "next_speaker": "moderator"
                            },
                            as_node="human_negative"
                        )
                        # Prepare to resume execution
                        current_inputs = None 
                    else:
                        # Some other interruption or unexpected state
